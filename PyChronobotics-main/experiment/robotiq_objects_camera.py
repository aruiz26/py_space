...
# mug_mesh.LoadWavefrontMesh(project_root + '/data/test_objs/newMug.obj', False, True)
...
# mug = chrono.ChBodyEasyCylinder(chrono.ChAxis_Y, 0.05, 0.1, 100, True, True, contact_material)
# mug.AddVisualShape(mug_shape)
# mug.SetName("mug")
# mug.SetPos(chrono.ChVector3d(0.03, 0.655, 0.1))
...
# cylinder = chrono.ChBodyEasyCylinder(chrono.ChAxis_Y, 0.05, 0.1, 10, True, True, contact_material)
# cylinder.GetVisualShape(0).SetTexture(chrono.GetChronoDataFile("textures/pink.png"))
# cylinder.SetName("cylinder")
# cylinder.SetPos(chrono.ChVector3d(0.03, 0.655, 0.1))
# cylinder.SetRot(chrono.Q_ROTATE_Y_TO_Z)
# cylinder.SetFixed(False)
# cylinder.EnableCollision(True)
# system.Add(cylinder)
# gripper.add_object("cylinder")
...
while vis.Run():
    ...
    if step_number % control_steps == 0:
        ...
        if 5 < sim_time < 7:
            ...
            desired_position = np.array([mug.GetPos().x + 0.02, mug.GetPos().y - 0.04, mug.GetPos().z + 0.05])
